...
function markerCircleFill(color2, context) {
  return create("svg:marker", context).attr("viewBox", "-5 -5 10 10").attr("markerWidth", 6.67).attr("markerHeight", 6.67).attr("fill", color2).attr("stroke", "white").attr("stroke-width", 1.5).call((marker) => marker.append("circle").attr("r", 3)).node();
}
function markerCircleStroke(color2, context) {
  return create("svg:marker", context).attr("viewBox", "-5 -5 10 10").attr("markerWidth", 6.67).attr("markerHeight", 6.67).attr("fill", "white").attr("stroke", color2).attr("stroke-width", 1.5).call((marker) => marker.append("circle").attr("r", 3)).node();
}
var nextMarkerId = 0;
...
function inferTickFormat(scale2, data, ticks2, tickFormat, anchor) {
  return typeof tickFormat === "function" ? tickFormat : tickFormat === void 0 && data && isTemporal(data) ? inferTimeFormat(data, anchor) ?? formatDefault : scale2.tickFormat ? scale2.tickFormat(typeof ticks2 === "number" ? ticks2 : null, tickFormat) : tickFormat === void 0 ? formatDefault : typeof tickFormat === "string" ? (isTemporal(scale2.domain()) ? utcFormat : format)(tickFormat) : constant(tickFormat);
}
    ...
    function postrender() {
      ...
      g.selectChildren().each(function(i) {
        ...
        if (a === void 0) {
          ...
          const x2 = px(i) + ox2;
          const y2 = py(i) + oy2;
          const fitLeft = x2 + w + r * 2 < width;
          const fitRight = x2 - w - r * 2 > 0;
          const fitTop = y2 + h + m + r * 2 + 7 < height;
...
function binn(bx, by, gx, gy, {
  ...
  return {
    ...
    ...basic(options, (data, facets, plotOptions) => {
      ...
      const bin2 = bing(bx == null ? void 0 : bx(data), by == null ? void 0 : by(data));
      let i = 0;
      for (const o of outputs)
        o.initialize(data);
      if (sort3)
...
var Rect = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X1 && X2 && x, y: Y1 && Y2 && y }, 0, 0).call(
      (g) => g.selectAll().data(index).enter().append("rect").call(applyDirectStyles, this).attr(
        "x",
        X1 && X2 && (projection2 || !isCollapsed(x)) ? (i) => Math.min(X1[i], X2[i]) + insetLeft : marginLeft + insetLeft
      ).attr(
        "y",
        Y1 && Y2 && (projection2 || !isCollapsed(y)) ? (i) => Math.min(Y1[i], Y2[i]) + insetTop : marginTop + insetTop
      ).attr(
        "width",
        X1 && X2 && (projection2 || !isCollapsed(x)) ? (i) => Math.max(0, Math.abs(X2[i] - X1[i]) - insetLeft - insetRight) : width - marginRight - marginLeft - insetRight - insetLeft
      ).attr(
        "height",
        Y1 && Y2 && (projection2 || !isCollapsed(y)) ? (i) => Math.max(0, Math.abs(Y1[i] - Y2[i]) - insetTop - insetBottom) : height - marginTop - marginBottom - insetTop - insetBottom
